...
| 지표 | 구현 가능 여부 |  지표 설명 |
| -- | -- | -- |
| 모멘텀점수 | O | 가격과 수급강도를 상대평가한 랭크점수 (높을수록 좋은 종목) |
| 모멘텀점수 과거값 | O |  n일 전 모멘텀 |
| 모멘텀점수 평균 | O |  모멘텀 n일 평균 |
| 모멘텀점수 변화량 | O |  n일 전 모멘텀 점수를 오늘과 비교 |
| 모멘텀점수 변화율 | O |  n일 전 모멘텀 점수를 오늘과 비율로 비교 |
| 모멘텀점수순위 | O |  모멘텀 점수 순위(랭킹) (낮을수록 좋은 종목) |
| 모멘텀점수순위 과거값 | O | n일 전 모멘텀 점수 순위 |
| 모멘텀점수순위 평균 | O | 모멘텀 점수 순위 n일 평균 |
| 모멘텀점수순위 변화량 | O | 모멘텀 점수 순위를 오늘과 비교 |
| 모멘텀점수순위 변화율 | O | 모멘텀 점수 순위를 오늘과 비율로 비교 |
| 업종모멘텀점수 | X | 해당 업종에 속한 모든 종목의 모멘텀 점수 평균값 (높을수록 좋은 종목) |
| 업종모멘텀점수 과거값 | X | n일 전 업종모멘텀점수 |
| 업종모멘텀점수 평균 | X | 업종모멘텀점수 n일 평균 |
| 업종모멘텀점수 변화량 | X | n일 전 업종 모멘텀 점수를 오늘과 비교 |
| 업종모멘텀점수 변화율 | X | n일 전 업종 모멘텀 점수를 오늘과 비율로 비교 |
| 업종모멘텀점수순위 | X | 업종 모멘텀 점수 순위(랭킹)|
| 업종모멘텀점수순위 과거값 | X |  |
| 업종모멘텀점수순위 평균 | X |  |
| 업종모멘텀점수순위 변화량 | X |  |
| 업종모멘텀점수순위 변화율 | X |  |
| 업종별모멘텀순위점수 | X | 업종별 모멘텀점수 순위를 산출하여 최저점에 가까울수록 높은 점수 부여 |
| 업종별모멘텀순위점수 과거값 | X |  |
| 업종별모멘텀순위점수 평균 | X |  |
| 업종별모멘텀순위점수 변화량 | X |  |
| 업종별모멘텀순위점수 변화율 | X |  |
| 모멘텀첫A | O | 최근 30일동안은 모멘텀 점수가 70점 아래에 있던 종목이 마침내 70점(상위 30%)을 돌파한 종목. 상승 초기에 있는 주도주를 찾는데 활용 가능 |
| 모멘텀첫A+ | O | 최근 30일동안은 모멘텀 점수가 90점 아래에 있던 종목이 마침내 90점(상위 10%)을 돌파한 종목. |
| 모멘텀A-연속 | O | 모멘텀 점수가 70이상을 몇일동안 유지하고 있는지 파악하는데 활용 |
| 모멘텀A+연속 | O | 모멘텀 점수가 90이상을 몇일동안 유지하고 있는지 파악하는데 활용.  |
| 베타 | X | 시장 대비 주가의 변동성을 나타내며 1보다 클 경우 시장보다 변동성이 더 높으며, 1보다 적을 경우 시장보다 변동성이 더 낮음 |

### 뉴지지표

| 지표 | 구현 가능 여부 |  지표 설명 |
| -- | -- | -- |
| 롱텀위치 | O | 2008년 신용위기(혹은 상장일 이후) 이후 현재 주가의 위치를 나타내며 0은 바닥, 100은 머리를 표시. 역사적 저점과 역사적 고점 위치를 정확히 알려준다 **이 지표를 사용할 때는 상장 후 경과일수 반드시 필요** |
| 롱텀기울기 | O | 롱텀차트상 가운데 기준선의 시작점과 끝점의 기울기(기간: 20081101 or 상장일부터): 각도=-90 ~ 90 |
| 숏텀위치(3개월) | O | 롱텀위치와 동일하지만 최근 3개월의 데이터 |
| 숏텀위치(1년6개월) | O | |
| 숏텀기울기(3개월) | O | |
| 숏텀기울기(1년6개월) | O | |
| 시즈널 | ? | 앞으로 3개월 내의 시즈널평균 상승 예측 점수 (변동폭이 작은 종목은 제외함) |
| 시즈널2 | ? | 기존 시즈널공식의 강화, 시즈널 하락구간에서는 매수가 안되도록 수정된 버전 |
